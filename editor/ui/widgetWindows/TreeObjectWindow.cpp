...
TreeObjectWindow::TreeObjectWindow() {
	...
	factory->signal_bind().connect([](const Glib::RefPtr<Gtk::ListItem> &pListItem) {
		...
		const auto col = std::dynamic_pointer_cast<EditorSceneObject>(row->get_item());
		if (!col) return;
...
void TreeObjectWindow::setTopLevelObjects(const std::shared_ptr<Gio::ListStore<EditorSceneObject>> &pTopLevelObjects) {
	tree.setSlotCreateModel(
		[this, pTopLevelObjects](const Glib::RefPtr<Glib::ObjectBase> &pItem) -> Glib::RefPtr<Gio::ListModel> {
			auto col = std::dynamic_pointer_cast<EditorSceneObject>(pItem);
			Glib::RefPtr<Gio::ListStore<EditorSceneObject>> result;
			if (col) {
				result = col->getChildrenUi();
			} else {
				result = pTopLevelObjects;
			}
