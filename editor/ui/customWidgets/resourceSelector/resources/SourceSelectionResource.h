...
protected:
	explicit ResourceElement(const std::shared_ptr<sdk::IResource> &pResource) : resource(pResource) {}

public:
	static std::shared_ptr<ResourceElement> create(const std::shared_ptr<sdk::IResource> &pResource) {
...
protected:
	explicit SourceSelectionResource(sdk::IResourceLoadExecutor* pLoader);
...
public:
	...
	virtual sdk::ResourceType getResourceType() = 0;

	virtual std::shared_ptr<SelectionElement> createElementFromResource(
		const std::shared_ptr<sdk::IResource> &pResource) = 0;

	[[nodiscard]] Gtk::ListView* getListView() const { return listView; }

protected:
	std::shared_ptr<Gtk::TreeListRow> findRow(const sdk::IResource* pResource) const;
