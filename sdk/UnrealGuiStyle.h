...
#define MYEDITOR_CONFIG_ONMERGE_OVERWRITE_WITH_SOURCE_LAYOUT                                                           \
	0 // Keep source (dragged) layout, vs keep target layout. Later seems to make more sense.
...
static void PushDocumentColors() {
	...
	ImGui::PushStyleColor(ImGuiCol_Tab,
						  ImLerp(ImVec4(0.26f + 0.40f, 0.59f, 0.98f, 0.31f), colors[ImGuiCol_TitleBgActive], 0.80f));
	ImGui::PushStyleColor(ImGuiCol_TabActive,
						  ImLerp(ImVec4(0.26f + 0.40f, 0.59f, 0.98f, 1.00f), colors[ImGuiCol_TitleBgActive], 0.60f));
...
enum MyEditorDocType { MyEditorType_None, MyEditorType_Scene, MyEditorType_Mesh };
...
static const char* g_EditorToolNames[] = {"Log", "Mesh", "Properties", "Extra"};
...
struct MyEditorDoc {
	...
	MyEditorDoc(MyEditorDocType type, const char* name) {
		...
		for (int n = 0; n < MyEditorToolType_Count_; n++) ToolAvail[n] = ToolVisible[n] = false;
	...
	void ResetToolsVisibility() {
		for (int n = 0; n < MyEditorToolType_Count_; n++) ToolAvail[n] = true;
...
static char* MyEditor_GetToolWindowName(MyEditorToolType tool_type, ImGuiID dockspace_id, char* buf, int buf_size) {
	...
	ImFormatString(buf, static_cast<size_t>(buf_size), "%s##%08X", tool_name, dockspace_id);
	return buf;
}
...
static const MyDummyAvailableDocument g_DummyAvailDocs[] = {
	...
	{"SCENE 1", MyEditorType_Scene, true}, {"SCENE 2", MyEditorType_Scene, true}, {"SCENE 3", MyEditorType_Scene, true},
	{"MESH 1", MyEditorType_Mesh, false},  {"MESH 2", MyEditorType_Mesh, false},  {"MESH 3", MyEditorType_Mesh, false},
};
...
struct MyEditor {
	...
	bool
		OptFloatingToolsOnlyVisibleWhenParentIsFocused; // Option: When multiple documents are open, floating tools only appear for focused one.
	...
	int GetDocumentsCount() const { return Docs.Size; }

	MyEditorDoc* FindDocumentByName(const char* name) {
		for (int n = 0; n < Docs.Size; n++)
			if (strcmp(name, Docs[n]->Name) == 0) return Docs[n];
		return NULL;
	}
};

enum MyEditorLayoutPreset { MyEditorLayoutPreset_Flat, MyEditorLayoutPreset_Standard };

static void MyEditor_LayoutPreset(MyEditorDoc* doc, ImGuiID dockspace_id, ImVec2 dockspace_size,
								  MyEditorLayoutPreset preset, ImGuiDir panel_dir) {
	...
	if (preset == MyEditorLayoutPreset_Standard) {
		...
		ImGui::DockBuilderDockWindow(
			MyEditor_GetToolWindowName(MyEditorToolType_Log, doc->CurrDockspaceID, buf, buf_size), dock_id_bottom);
		ImGui::DockBuilderDockWindow(
			MyEditor_GetToolWindowName(MyEditorToolType_Properties, doc->CurrDockspaceID, buf, buf_size), dock_id_prop);
		ImGui::DockBuilderDockWindow(
			MyEditor_GetToolWindowName(MyEditorToolType_Mesh, doc->CurrDockspaceID, buf, buf_size), dock_id_prop);
		ImGui::DockBuilderDockWindow(
...
static void MyEditor_OpenDocumentSelector(MyEditor* editor) {
	...
	if (avail_count == 0) ImGui::TextDisabled("<None>");
}
...
static void MyEditor_TopLevelMenuBar(MyEditor* editor, MyEditorDoc* doc, ImGuiID dockspace_id, ImVec2 dockspace_size) {
	...
	if (ImGui::BeginMenu("File")) {
		...
		if (ImGui::MenuItem("Save", "Ctrl+S")) doc->Save();
		ImGui::MenuItem("Save As", "Ctrl+Shift+S");
		ImGui::Separator();
		if (ImGui::MenuItem("Close", "Ctrl+W", false, doc != NULL)) doc->Open = false;
		ImGui::Separator();
	...
	if (ImGui::BeginMenu("Windows")) {
		...
		if (doc != NULL) {
			for (int n = 0; n < MyEditorToolType_Count_; n++)
				if (doc->ToolAvail[n])
					if (ImGui::MenuItem(g_EditorToolNames[n], NULL, &doc->ToolVisible[n], menu_enabled)) {
						//
						std::cout << std::endl;
					}
			ImGui::Separator();
		...
		if (ImGui::BeginMenu("Layout", menu_enabled)) {
			...
			if (ImGui::MenuItem("*Debug: Clear Layout")) {
				...
				ImGui::DockBuilderRemoveNodeDockedWindows(
					dock_main_id); // Remove everything, including windows we don't know about (e.g. Demo Window)
				ImGui::DockBuilderFinish(dockspace_id);
			}
			...
			if (ImGui::MenuItem("*Debug: Merge all nodes")) {
				...
				ImGui::DockBuilderRemoveNodeChildNodes(
					dock_main_id); // Anything docked into any split child node will be moved to the root of the dockspace.
				ImGui::DockBuilderFinish(dockspace_id);
			}
...
static void MyEditor_UpdatePlatformTitleBar(MyEditor* editor, MyEditorDoc* doc) {
	...
	if (viewport->PlatformWindowCreated && (viewport->Flags & ImGuiViewportFlags_NoDecoration) == 0 &&
		platform_io.Platform_SetWindowTitle) {
		char os_window_title[128];
		ImFormatString(os_window_title, IM_ARRAYSIZE(os_window_title), "%s",
					   doc->Name); // Here one could e.g. strip icons
...
static void MyEditor_UpdateDocLocation(MyEditor* editor, MyEditorDoc* doc, ImGuiID toplevel_dockspace_id) {
	...
	if (doc->Unsaved) window_flags |= ImGuiWindowFlags_UnsavedDocument;
	PushDocumentColors();
	...
	if (visible && editor->LastActiveDocument == doc) MyEditor_UpdatePlatformTitleBar(editor, doc);
	...
	doc->Window = ImGui::GetCurrentWindow();
	...
	doc->ToolWindowsClass.ParentViewportId =
		ImGui::GetWindowViewport()->ID;								  // Make child of the top-level editor window
...
static void MyEditor_UpdateDocContents(MyEditor* editor, MyEditorDoc* doc) {
	...
	if (doc->PrevLocationID != 0 && doc->PrevLocationID != doc->CurrLocationID) {
		...
		for (int doc_n = 0; doc_n < editor->Docs.Size; doc_n++) {
			...
			if (other_doc->CurrDockspaceID == doc->PrevDockspaceID) prev_dockspace_ref_count++;
			if (other_doc->CurrDockspaceID == doc->CurrDockspaceID) curr_dockspace_ref_count++;
		}
		...
		if (curr_dockspace_ref_count <= 1) MyEditor_LayoutCopy(doc->PrevDockspaceID, doc->CurrDockspaceID);
#endif
	...
	for (int tool_type = 0; tool_type < MyEditorToolType_Count_; tool_type++) {
		if (!doc->ToolAvail[tool_type] || !doc->ToolVisible[tool_type]) continue;
		...
		if (editor->OptFloatingToolsOnlyVisibleWhenParentIsFocused && !is_last_focused_doc)
			if (ImGuiWindow* window = ImGui::FindWindowByName(buf)) {
				...
				if (window_node == NULL || ImGui::DockNodeGetRootNode(window_node)->ID != dockspace_id) continue;
			}

		const ImGuiWindowFlags tools_shared_window_flags =
		...
		if (ImGui::SmallButton("Dummy Button")) doc->Unsaved = true;
		...
		if (tool_type == MyEditorToolType_Properties)
			ImGui::ColorEdit4(
				"Color editor", &doc->DummyStateColor.x,
				ImGuiColorEditFlags_NoInputs); // This is useful to test because popup viewport parent/child relation may incorrectly affect focus.
...
[[maybe_unused]] static void MyEditor_ShowDemo(MyEditor* editor) {
	...
	if (editor->DocsToOpen.Size > 0) {
		for (auto doc: editor->DocsToOpen) editor->Docs.push_back(doc);
	...
	ImGuiID root_dockspace_id = 0;
	{
		...
		ImGui::DockSpace(root_dockspace_id, ImVec2(0, 0),
						 ...
						 &editor->TopLevelEditorWindowClass);
		...
		if (open_count == 0) MyEditor_TopLevelMenuBar(editor, NULL, 0, ImVec2(0, 0));
	...
	for (int n = 0; n < editor->Docs.Size; n++) {
		...
		if (doc->Open == false) {
			IM_DELETE(doc);
			if (editor->LastActiveDocument == doc) editor->LastActiveDocument = NULL;
