...
public:
	template <typename T>
	static std::string getTypeName(const T &pE) { return demangle(typeid(pE).name()); }

	template <typename T>
	static std::string getTypeName(T* pE) { return demangle(typeid(*pE).name()); }

	template <typename T>
	static std::string getTypeName() {
		std::string result;
		if constexpr (std::is_const_v<std::remove_reference_t<T>>) result += "const ";
		if constexpr (std::is_pointer<T>()) { result += demangle(typeid(std::remove_pointer_t<T>).name()); } else
